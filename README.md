...
两边通过 `ceclientbridge:*` 插件消息通道通信，必须配套使用。物品、方块、酿造、合成和锻造数据使用独立的分块同步通道；26.2 Jade 额外使用 `block_icons`、`furniture_probe` 和 `furniture_icon` 通道。

## Jade 26.2 如何显示 CraftEngine 方块和家具

客户端在世界里看到的 CraftEngine 方块仍然是音符盒、绊线等原版视觉状态，Jade 因此默认只能显示原版图标和名称。本桥接不会硬编码某一种方块，而是按下面的链路恢复真实外观：
...
3. 26.2 客户端收到 `block_icons` 后，以视觉状态为键缓存重建出的 `ItemStack`。Jade 查询方块时直接调用自己的物品渲染器显示该物品，而不是尝试手工绘制模型。
...
家具不能只靠客户端实体类型判断。Jade 首次注视一个实体时，26.2 客户端发送带请求 ID 的 `furniture_probe`；服务端按实体 ID 查询 CraftEngine 的元数据实体、碰撞体或座位，校验世界和距离后，把家具的来源物品（必要时重新构建家具物品）转换为客户端外观并返回。客户端校验请求 ID 后缓存图标；未识别实体会被负缓存，请求还有超时重试和服务端每玩家限流。家具名称继续使用 Jade/CraftEngine 实体本身已经正确的名称，不做额外覆盖。

这些 Jade 图标和标题增强目前只在 Minecraft 26.2 客户端实现，`client-legacy/` 的 1.21.11 客户端不包含这套新通道。

## 依赖要求

- 服务端：Paper/ASPaper，Minecraft 26.2（26.x 服务端构建基线），已安装并加载 [CraftEngine](https://craftengine.net/)。
...
本仓库自身代码使用 [MIT 许可证](LICENSE)。CraftEngine 及仓库中的社区版 CraftEngine 开发依赖都是独立的第三方软件，不受本项目 MIT 许可证覆盖；仓库中的开发 JAR 仅用于 `compileOnly` 构建，运行服务器仍需自行提供并安装合适版本的 CraftEngine。
